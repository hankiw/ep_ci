						...
						<div class="field-body">
							<div class="field">
								<p class="control is-expanded has-icons-left">
									<input class="input is-small p-1" type="month" name="YM" placeholder="" value="<?=$YMF?>" size="5" manlength="6" onchange="selectYM();">
								</p>
							</div>
							<div class="field">
								<p class="control" style="width:50px;">
									<input class="input is-small p-1" type="text" name="DD" placeholder="" value="<?=$DD?>" size="3" manlength="2" onchange="selectYM();">
								</p>
							</div>
						</div>
					</div>
		...
		<div>
			...
			<script>
				...
				document.addEventListener('DOMContentLoaded', function() {
					...
					if (DVS == '6') {
						let tmpArr = [];
						masterColumns.forEach(function(obj, idx) {
							if (['TYP', 'GBN', 'TRS', 'EBL', 'ENO'].indexOf(obj.name) < 0) tmpArr.push(obj);
						});
						masterColumns = tmpArr;
					} else if (DVS == '7') {
						let tmpArr = [];
						masterColumns.forEach(function(obj, idx) {
							if (['TYP', 'GBN', 'TRS', 'EBL', 'ENO'].indexOf(obj.name) < 0) tmpArr.push(obj);
					...
					if (DVS == '6') {
						let tmpArr = [];
						detailColumns.forEach(function(obj, idx) {
							if (['RETN', 'PAMT', 'BNFT', 'BNFT_DESC'].indexOf(obj.name) < 0) tmpArr.push(obj);
						});
						detailColumns = tmpArr;
					} else if (DVS == '7') {
						let tmpArr = [];
						detailColumns.forEach(function(obj, idx) {
							if (['RETN', 'PAMT', 'BNFT', 'BNFT_DESC'].indexOf(obj.name) < 0) tmpArr.push(obj);
					...
					if (DVS == '1') {
						...
						gridViewDetail.columnByName("PAMT").header.text = "판매금액";
					}
